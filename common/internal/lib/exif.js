 ...
 * @param {Img} image 
 */
var detectVerticalSquash = function detectVerticalSquash(image) {
 ...
 */
export function readOrientation(file, fileType) {
  return new Promise(function (resolve) {
    // if (fileType !== 'jpg') return done(1)
    var reader = new FileReader();
    reader.onload = function () {
      return resolve(readExif(reader.result));
    };
 ...
 */
export function vaildImgType(blob) {
  return new Promise(function (resolve) {
    var reader = new FileReader();
    reader.onload = function () {
      var buf = new DataView(reader.result);
      resolve(buf.byteLength > 1 ? {
        'ffd8': 'jpg',
        '8950': 'png',
        '4749': 'gif',
        '424d': 'bmp'
 ...
 * @param {Int} maxWidth
 */
export function compressImg(img, orientation, maxWidth) {
  return new Promise(function (resolve) {
    ...
    // 当ios7 ／ ios6 的时候算压缩比
    if (is.ios && ~[7, 6].indexOf(parseInt(navigator.userAgent.toLowerCase().match(/os (\d+)_\d+/)[1]))) {
